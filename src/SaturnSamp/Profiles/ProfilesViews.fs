...
module Views =
  let index (ctx : HttpContext) (objs : Profile list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing Profiles"]
        ...
        table [_class "table is-hoverable is-fullwidth"] [
          thead [] [
            tr [] [
              ...
              th [] [rawText "Name"]
              th [] [rawText "Position"]
              th [] [rawText "EmailAddress"]
              th [] [rawText "PhoneNumber"]
              th [] [rawText "Contacted"]
              th [] []
            ]
          ]
          tbody [] [
            for o in objs do
              yield tr [] [
                ...
                td [] [rawText (string o.position)]
                td [] [rawText (string o.emailAddress)]
                td [] [rawText (string o.phoneNumber)]
                td [] [rawText (string o.contacted)]
                td [] [
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New Profile"]
      ]
    ]
    App.layout ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : Profile) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show Profile"]

        ul [] [
          li [] [ strong [] [rawText "Id: "]; rawText (string o.id) ]
          li [] [ strong [] [rawText "Name: "]; rawText (string o.name) ]
          li [] [ strong [] [rawText "Position: "]; rawText (string o.position) ]
          li [] [ strong [] [rawText "EmailAddress: "]; rawText (string o.emailAddress) ]
          li [] [ strong [] [rawText "PhoneNumber: "]; rawText (string o.phoneNumber) ]
          li [] [ strong [] [rawText "Contacted: "]; rawText (string o.contacted) ]
        ]
        a [_class "button is-text"; _href (Links.edit ctx o.id)] [rawText "Edit"]
  ...
  let private form (ctx: HttpContext) (o: Profile option) (validationResult : Map<string, string>) isUpdate =
    ...
    let cnt = [
      div [_class "container "] [
        form [ _action (if isUpdate then Links.withId ctx o.Value.id else Links.index ctx ); _method "post"] [
          ...
          yield field (fun i -> (string i.position)) "Position" "position" 
          yield field (fun i -> (string i.emailAddress)) "EmailAddress" "emailAddress" 
          yield field (fun i -> (string i.phoneNumber)) "PhoneNumber" "phoneNumber" 
          yield field (fun i -> (string i.contacted)) "Contacted" "contacted" 
          yield buttons
        ]
      ]
    ]
  ...
  let add (ctx: HttpContext) (o: Profile option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: Profile) (validationResult : Map<string, string>) =
